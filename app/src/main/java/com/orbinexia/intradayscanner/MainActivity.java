...
public class MainActivity extends Activity {
    ...
    private void scanAllSafe(){
        new Thread(()->{
            ...
            final int n=ok;
            runOnUiThread(()->{status.setText("최근 스캔: "+new java.text.SimpleDateFormat("HH:mm:ss",Locale.KOREA).format(new Date())+" · 성공 "+n+"/"+watchlist.size());render();});
        }).start();
    }
    ...
    private ScanRow scanOne(WatchItem w)throws Exception{
        ...
        for(int i=0;i<ca.length();i++){
            if(ca.isNull(i)||ha.isNull(i)||la.isNull(i)||va.isNull(i))continue;
            close.add(ca.getDouble(i));high.add(ha.getDouble(i));low.add(la.getDouble(i));vol.add(va.getDouble(i));
        }
    ...
    private void classify(ScanRow r){
        ...
        if(above){why.add("VWAP 상단");risk--;}if(hot)why.add("거래량 급증");if(over){why.add("RSI 과열");risk+=2;}if(weak){why.add("RSI 약세");risk+=2;}if(very){why.add("변동성 높음");risk+=2;}
        r.risk=Math.max(1,Math.min(10,risk));r.reasons=android.text.TextUtils.join(" · ",why);
        if(trend&&above&&hot&&!over)r.signal="ENTRY_WATCH";else if(trend&&!weak)r.signal="HOLD_WATCH";else if(over||very||weak)r.signal="RISK_RISING";else r.signal="WAIT";
    }
